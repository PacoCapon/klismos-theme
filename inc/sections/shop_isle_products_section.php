...
while ( $shop_isle_latest_loop->have_posts() ) {
	...
	if ( has_post_thumbnail( $shop_isle_latest_loop->post->ID ) ) {
		echo "<img src='".get_the_post_thumbnail_url( $shop_isle_latest_loop->post->ID, 'shop_catalog' )."' title='".get_the_title()."'>";

		if ( $shop_isle_gallery_attachment_ids ) {
			echo wp_get_attachment_image_no_srcset(
				$shop_isle_gallery_attachment_ids[0],
				'shop_catalog',
				false,
				array(
					'title' => '',
					'alt'   => '',
					'class' => 'shop-isle-second-image',
				)
			);
		}
	...
	if ( ! empty( $product ) ) {
		...
		echo '</div>';
		echo '</div>';
		echo '<h4 class="shop-item-title font-alt"><a href="' . esc_url( get_permalink() ) . '" tartget="blank">' . get_the_title() . '</a></h4>';
		$rating_html = '';
